 ...
 ******************************************************************************/
#if  0
unsigned int  ReadLoadRes(uint8_t ch)   
{  
	...
	else
	{
		...
		TEST_LOAD2_ON;	//开启检测电路


		LoadValue = HAL_ADC_Sample(ADC_CH_VOUT2,True);	//延时丢掉采样值，等待稳定
		LoadValue = HAL_ADC_Sample(ADC_CH_VOUT2,True);
		TEST_LOAD2_OFF;	//检测完成关闭检测电路
